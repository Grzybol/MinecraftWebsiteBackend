...
func GetBackpackHandler(c *gin.Context) {
	handleEquipment(c, "backpack")
}

func GetArmorHandler(c *gin.Context) {
	handleEquipment(c, "armor")
}

func GetHotbarHandler(c *gin.Context) {
	handleEquipment(c, "hotbar")
}

func handleEquipment(c *gin.Context, kind string) {
	...
	socketPath := fmt.Sprintf("/tmp/plugin_%s_%s.sock", serverKey, kind)
	...
	ElasticWriter.WriteWithIP(fmt.Sprintf("🔌 EQ Request → kind: %s | server: %s | player: %s | socket: %s | token: %s", kind, serverKey, playerName, socketPath, token), ip)

	jsonResponse, err := getSocketResponse(socketPath, token, playerName, ip)
	...
	ElasticWriter.WriteWithIP(fmt.Sprintf("✅ Odpowiedź z socketa (%s): %s", kind, truncateForLog(jsonResponse, 100)), ip)

	c.Data(200, "application/json", []byte(jsonResponse))
}

func getSocketResponse(socketPath, token, playerName, ip string) (string, error) {
...
// Truncator do logów (opcjonalny helper)
